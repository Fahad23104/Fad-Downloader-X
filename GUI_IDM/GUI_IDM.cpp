...
//      Fixes: Winsock2 Clash & Linker Subsystems
// ==========================================

#define _CRT_SECURE_NO_WARNINGS // Silences MSVC string warnings

// 1. LINKING & SUBSYSTEM OVERRIDE
#pragma comment(lib, "ws2_32.lib")
#pragma comment(lib, "crypt32.lib")
#pragma comment(lib, "wldap32.lib")
#pragma comment(lib, "Normaliz.lib") 
#pragma comment(linker, "/SUBSYSTEM:WINDOWS") // Forces GUI mode to fix "unresolved external symbol main"

// 2. INCLUDES
#include <winsock2.h> // MUST be included before any other Windows headers to prevent API clashes
#include <wx/wx.h>
...
HANDLE g_InstanceMutex = NULL; // The Global Kernel Lock
...
// --- WORKER THREAD ---
struct WriteData {
    FILE* fp; int task_id; int thread_index; long long disk_offset; std::vector<char> ram_buffer;
};
...
void StartTaskLogic(int task_id) {
    ...
    {
        std::lock_guard<std::mutex> lock(tasks_mutex);
        task->active_threads = task->thread_states.size();
    ...
    : wxFrame(parent, wxID_ANY, "Downloading...", wxDefaultPosition, wxSize(550, 350), wxDEFAULT_FRAME_STYLE | wxFRAME_FLOAT_ON_PARENT), task_id(tid)
{
    ...
    lblUrl = new wxStaticText(panel, wxID_ANY, "URL: " + display_url); lblUrl->SetFont(fontNormal);
    ...
    lblPath = new wxStaticText(panel, wxID_ANY, "Saving to: " + display_path); lblPath->SetFont(fontNormal);
    panelSizer->Add(lblPath, 0, wxALL | wxEXPAND, 8);

    wxBoxSizer* statusSizer = new wxBoxSizer(wxHORIZONTAL);
    st1 = new wxStaticText(panel, wxID_ANY, "Status: "); st1->SetFont(fontNormal);
    statusSizer->Add(st1, 0, wxALIGN_CENTER_VERTICAL);

    lblStatus = new wxStaticText(panel, wxID_ANY, "Initializing..."); lblStatus->SetFont(fontBold);
    ...
    st2 = new wxStaticText(panel, wxID_ANY, "File Size:"); lblTotalSize = new wxStaticText(panel, wxID_ANY, task ? format_bytes(task->size) : "Calculating...");
    grid->Add(st2); grid->Add(lblTotalSize);

    st3 = new wxStaticText(panel, wxID_ANY, "Downloaded:"); lblDownloaded = new wxStaticText(panel, wxID_ANY, "0 B");
    grid->Add(st3); grid->Add(lblDownloaded);

    st4 = new wxStaticText(panel, wxID_ANY, "Transfer Rate:"); lblSpeed = new wxStaticText(panel, wxID_ANY, "0 KB/sec");
    grid->Add(st4); grid->Add(lblSpeed);

    st5 = new wxStaticText(panel, wxID_ANY, "Time Left:"); lblTimeLeft = new wxStaticText(panel, wxID_ANY, "Calculating...");
    ...
    btnSizer->Add(btnPause, 0, wxRIGHT, 10); btnSizer->Add(btnHide, 0, wxRIGHT, 10); btnSizer->Add(btnCancelDl, 0);
...
void DownloadProgressWindow::OnTimer(wxTimerEvent& event) {
    ...
    if (task->is_completed) {
        ...
        btnPause->Disable(); btnCancelDl->Disable();
        progressGauge->SetValue(100); timer->Stop();
    }
}
...
void DownloadProgressWindow::OnCancelDownload(wxCommandEvent& event) {
    ...
    std::thread([id]() {
        ...
        auto it = std::remove_if(all_tasks.begin(), all_tasks.end(), [id](std::shared_ptr<DownloadTask> t) {
            if (t->id == id) {
                if (!t->filename.empty()) { std::remove(t->filename.c_str()); std::string state_file = t->filename + ".idm"; std::remove(state_file.c_str()); }
                return true;
            } return false;
            });
...
public:
    ...
    void ShowAddDialog(const std::string& prefill_url);

private:
    void OnAdd(wxCommandEvent& event); void OnTimer(wxTimerEvent& event); void CheckClipboard();
    void ApplyTheme(); void OnToggleTheme(wxCommandEvent& event);
    std::vector<long> GetSelectedRows(); void UpdateToolbarState();
    void OnItemDoubleClicked(wxListEvent& event); void OnItemSelected(wxListEvent& event); void OnItemDeselected(wxListEvent& event);
    void OnRightClick(wxListEvent& event); void OnContextResume(wxCommandEvent& event); void OnContextPause(wxCommandEvent& event);
    void OnContextDelete(wxCommandEvent& event); void OnDeleteAll(wxCommandEvent& event); void OnContextOpenFolder(wxCommandEvent& event);
    void OnContextProperties(wxCommandEvent& event); void OnContextRefreshLink(wxCommandEvent& event);
    ...
    enum {
        ...
        ID_CTX_OPEN_FOLDER = 204, ID_CTX_PROPS = 205, ID_CTX_REFRESH = 206, ID_DELETE_ALL = 207, ID_THEME_TOGGLE = 301
    };
};
...
bool IDMApp::OnInit() {
    // FIX: WINDOWS GLOBAL KERNEL MUTEX FOR PERFECT CHROME IPC ISOLATION
    ...
    if (GetLastError() == ERROR_ALREADY_EXISTS) {
        if (argc > 1) {
            ...
            int retries = 10;
            while (retries-- > 0) {
                std::ofstream out(GetExeDir() + "\\fdx_queue.txt", std::ios::app);
                if (out.is_open()) {
                    out << passed_url << "\n";
                    out.close();
                    break;
...
void MainFrame::ApplyTheme() {
    if (g_is_dark_mode) {
        ...
        listView->SetBackgroundColour(darkListBg); listView->SetForegroundColour(darkText); listView->SetTextColour(darkText); listView->EnableAlternateRowColours(true);
        ...
        listView->SetBackgroundColour(lightBg); listView->SetForegroundColour(lightText); listView->SetTextColour(lightText); listView->EnableAlternateRowColours(true);
...
std::vector<long> MainFrame::GetSelectedRows() {
    std::vector<long> rows; long item = -1;
    for (;;) { item = listView->GetNextItem(item, wxLIST_NEXT_ALL, wxLIST_STATE_SELECTED); if (item == -1) break; rows.push_back(item); }
    return rows;
}

void MainFrame::UpdateToolbarState() {
    auto rows = GetSelectedRows();
    if (rows.empty()) { btnRes->Enable(false); btnStop->Enable(false); btnDel->Enable(false); return; }
...
void MainFrame::OnItemDoubleClicked(wxListEvent& event) {
    long row = event.GetIndex(); if (row == -1) return;
    int id = std::atoi(listView->GetItemText(row, 0).c_str());
    std::shared_ptr<DownloadTask> task = nullptr;
    { std::lock_guard<std::mutex> lock(tasks_mutex); for (auto& t : all_tasks) if (t->id == id) task = t; }
    if (task && task->is_completed && !task->filename.empty()) { if (wxFileExists(task->filename)) wxLaunchDefaultApplication(task->filename); }
}
...
void MainFrame::ShowAddDialog(const std::string& prefill_url) {
    ...
    if (urlDialog.ShowModal() == wxID_OK) {
        ...
        std::atomic<bool> fetch_done{ false }; FileInfo info;
        std::thread([&]() { info = GetRobustFileInfo(url); fetch_done = true; }).detach();
        while (!fetch_done) { fetchDialog.Pulse(); std::this_thread::sleep_for(std::chrono::milliseconds(50)); wxYield(); }
        fetchDialog.Update(100);
        ...
        int new_id = g_id_counter++; auto task = std::make_shared<DownloadTask>(new_id, url);
        task->filename = savePath; task->short_name = shortName; task->size = info.size;
        ...
        listView->SetItem(index, 1, task->short_name); listView->SetItem(index, 3, "Pending");

        std::thread(StartTaskLogic, new_id).detach();
        DownloadProgressWindow* dlg = new DownloadProgressWindow(this, new_id); dlg->Show();
    }
}
...
void MainFrame::OnAdd(wxCommandEvent& event) {
    ...
    if (wxTheClipboard->Open()) {
        if (wxTheClipboard->IsSupported(wxDF_TEXT)) {
            ...
            if (IsDownloadableLink(text)) clipUrl = text;
        }
    ...
    if (!m_is_dialog_open) {
        ...
        ShowAddDialog(clipUrl);
        m_is_dialog_open = false;
    }
    else {
        if (!clipUrl.empty()) m_pending_urls.push_back(clipUrl);
    }
}
            ...
            if (wxTheClipboard->GetData(data)) {
                ...
                if (!text.empty() && text != last_clipboard_text && IsDownloadableLink(text)) {
                    if (std::find(m_processed_urls.begin(), m_processed_urls.end(), text) == m_processed_urls.end()) {
                        m_pending_urls.push_back(text);
                        m_processed_urls.push_back(text);
                        if (m_processed_urls.size() > 50) m_processed_urls.pop_front();
                    }
...
void MainFrame::OnRightClick(wxListEvent& event) {
    ...
    bool can_resume = false; bool can_pause = false;
    auto rows = GetSelectedRows();
    {
        std::lock_guard<std::mutex> lock(tasks_mutex);
        for (long r : rows) {
    ...
    wxMenu menu;
    menu.Append(ID_CTX_PROPS, "Show Progress Window"); menu.AppendSeparator();
    menu.Append(ID_CTX_RESUME, "Resume Download"); menu.Append(ID_CTX_PAUSE, "Stop Download"); menu.Append(ID_CTX_REFRESH, "Refresh Download Address");
    menu.AppendSeparator(); menu.Append(ID_CTX_OPEN_FOLDER, "Open Folder"); menu.Append(ID_CTX_DELETE, "Remove");
    menu.Enable(ID_CTX_RESUME, can_resume); menu.Enable(ID_CTX_PAUSE, can_pause);
    PopupMenu(&menu);
}
...
void MainFrame::OnContextResume(wxCommandEvent& event) {
    ...
    for (long row : rows) {
        ...
        std::shared_ptr<DownloadTask> task = nullptr;
        { std::lock_guard<std::mutex> lock(tasks_mutex); for (auto& t : all_tasks) if (t->id == id) task = t; }
        if (task && !task->is_running && !task->is_completed) std::thread(StartTaskLogic, id).detach();
    }
    UpdateToolbarState();
}
...
void MainFrame::OnContextPause(wxCommandEvent& event) {
    auto rows = GetSelectedRows();
    { std::lock_guard<std::mutex> lock(tasks_mutex); for (long row : rows) { int id = std::atoi(listView->GetItemText(row, 0).c_str()); for (auto& t : all_tasks) if (t->id == id) t->is_paused = true; } }
    UpdateToolbarState();
}

void MainFrame::OnContextRefreshLink(wxCommandEvent& event) {
    auto rows = GetSelectedRows(); if (rows.empty()) return;
    ...
    if (task) {
        ...
        if (urlDialog.ShowModal() == wxID_OK) {
            std::lock_guard<std::mutex> lock(tasks_mutex);
            task->url = urlDialog.GetValue().ToStdString(); task->error = false; task->status_text = "Address Updated"; task->is_paused = true;
        }
    }
}

void MainFrame::ExecuteDeletion(const std::vector<int>& ids_to_delete, bool delete_from_disk) {
    { std::lock_guard<std::mutex> lock(tasks_mutex); for (int id : ids_to_delete) { for (auto& t : all_tasks) if (t->id == id) t->is_paused = true; } }
    for (int id : ids_to_delete) {
        for (int i = 0; i < listView->GetItemCount(); i++) {
            if (std::atoi(listView->GetItemText(i, 0).c_str()) == id) { listView->DeleteItem(i); break; }
        }
    }
    UpdateToolbarState();
        ...
        auto it = std::remove_if(all_tasks.begin(), all_tasks.end(),
            [&](std::shared_ptr<DownloadTask> t) {
                if (std::find(ids_to_delete.begin(), ids_to_delete.end(), t->id) != ids_to_delete.end()) {
                    if (!t->filename.empty()) { std::string state_file = t->filename + ".idm"; std::remove(state_file.c_str()); if (delete_from_disk) std::remove(t->filename.c_str()); }
                    return true;
                } return false;
            });
        all_tasks.erase(it, all_tasks.end());
...
void MainFrame::OnContextDelete(wxCommandEvent& event) {
    auto rows = GetSelectedRows(); if (rows.empty()) return;
    wxMessageDialog dlg(this, "Delete the downloaded file(s) from your hard drive as well?\n\nYes = Remove from List & Disk\nNo = Remove from List Only", "Confirm Deletion", wxYES_NO | wxCANCEL | wxICON_QUESTION);
    int result = dlg.ShowModal(); if (result == wxID_CANCEL) return;
    std::vector<int> ids; for (long row : rows) ids.push_back(std::atoi(listView->GetItemText(row, 0).c_str()));
    ExecuteDeletion(ids, (result == wxID_YES));
}

void MainFrame::OnDeleteAll(wxCommandEvent& event) {
    if (listView->GetItemCount() == 0) return;
    wxMessageDialog dlg(this, "Are you sure you want to remove ALL downloads?\n\nYes = Remove ALL from List & Disk\nNo = Remove ALL from List Only", "Delete All Downloads", wxYES_NO | wxCANCEL | wxICON_WARNING);
    int result = dlg.ShowModal(); if (result == wxID_CANCEL) return;
    std::vector<int> ids; for (int i = 0; i < listView->GetItemCount(); i++) ids.push_back(std::atoi(listView->GetItemText(i, 0).c_str()));
    ExecuteDeletion(ids, (result == wxID_YES));
}
...
void MainFrame::OnContextProperties(wxCommandEvent& event) {
    auto rows = GetSelectedRows(); if (rows.empty()) return;
    int id = std::atoi(listView->GetItemText(rows[0], 0).c_str());
    DownloadProgressWindow* dlg = new DownloadProgressWindow(this, id); dlg->Show();
}
...
void MainFrame::OnContextOpenFolder(wxCommandEvent& event) {
    ...
    std::shared_ptr<DownloadTask> task = nullptr;
    { std::lock_guard<std::mutex> lock(tasks_mutex); for (auto& t : all_tasks) if (t->id == id) task = t; }
    if (task && !task->filename.empty()) { std::string cmd = "explorer /select,\"" + task->filename + "\""; system(cmd.c_str()); }
    else { system("explorer ."); }
}

void MainFrame::OnTimer(wxTimerEvent& event) {
    CheckClipboard();

    std::string queuePath = GetExeDir() + "\\fdx_queue.txt";
        ...
        if (in.is_open()) {
            ...
            while (std::getline(in, line)) {
                ...
                if (!line.empty() && std::find(m_processed_urls.begin(), m_processed_urls.end(), line) == m_processed_urls.end()) {
                    new_urls.push_back(line);
                    m_processed_urls.push_back(line);
                    if (m_processed_urls.size() > 50) m_processed_urls.pop_front();
                }
            }
            in.close();
            ...
            for (int i = 0; i < 5; i++) {
                if (std::remove(queuePath.c_str()) == 0) break;
                wxMilliSleep(10);
            }
            for (const auto& u : new_urls) m_pending_urls.push_back(u);
        }
    }

    if (!m_is_dialog_open && !m_pending_urls.empty()) {
        std::string next_url = m_pending_urls.front();
        m_pending_urls.pop_front();

        m_is_dialog_open = true;
        this->CallAfter([this, next_url]() {
            Restore();
            Raise();
            ShowAddDialog(next_url);
            m_is_dialog_open = false;
            });
    }
    ...
    for (int id : to_remove_ui) {
        for (int i = 0; i < listView->GetItemCount(); i++) {
            if (std::atoi(listView->GetItemText(i, 0).c_str()) == id) { listView->DeleteItem(i); toolbar_needs_update = true; break; }
        }
    }
            ...
            if (task) {
                ...
                if (task->size > 0) {
                    ...
                    if (task->is_running && !task->is_completed) {
                        ...
                        std::stringstream ss; ss << std::fixed << std::setprecision(2) << avg << " MB/s";
                        listView->SetItem(i, 5, ss.str()); task->last_bytes = task->downloaded;
                    }
                    else {
                        listView->SetItem(i, 5, ""); task->current_speed = 0;
                    }
                }
            }
        }
